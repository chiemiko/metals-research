...
def regression_plot(y_hat, y, model_name, offset):
    ...
    if offset != 0:
        ...
        plt.savefig(str(model_name) + '_' + 'same_day_preds.png', bbox_inches = 'tight')
...
def fit_predict(model, X_train, X_test, y_train, y_test, train_tail, window_setting, offset):
    ...
    1) Fits model on training data
    2) Makes predictions on test data
    3) Calls reverse_stationarity function on prediction (y_pred) and actual (y) variables
    """
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    y_pred = pd.Series(y_pred)
...
if __name__ == "__main__":
    main(sys.argv[1:])
>>>>>>> 18d4ecb7dbb9c72f3d82e5b9985e259cdf087fa5
